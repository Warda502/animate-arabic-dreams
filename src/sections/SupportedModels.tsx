...
const SupportedModels = () => {
  ...
  const filteredModels = models.filter(model => model.model.toLowerCase().includes(searchQuery.toLowerCase()) || model.carrier && model.carrier.toLowerCase().includes(searchQuery.toLowerCase()) || model.security && model.security.toLowerCase().includes(searchQuery.toLowerCase()) || model.operation && model.operation.toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  return <div className="pt-24 pb-16 py-0">
      ...
      <section className="py-10">
        <div className="container mx-auto px-4">
          ...
          Array(8).fill(0).map((_, index) => <Card key={`loading-${index}`} className="h-40 animate-pulse">
                  <CardContent className="flex items-center justify-center h-full">
                    <div className="w-12 h-12 rounded-full bg-gray-200 dark:bg-gray-700"></div>
                  </CardContent>
                </Card>) : brands.length > 0 ? brands.map(brand => <Card key={brand} className={`cursor-pointer transition-all duration-300 hover:shadow-lg ${selectedBrand === brand ? 'bg-orange-50 dark:bg-orange-900/30 border-orange-300 dark:border-orange-600' : 'hover:bg-orange-50 dark:hover:bg-gray-800'}`} onClick={() => handleBrandSelect(brand)}>
      ...
      {/* Models Section */}
      {selectedBrand && <section className="py-0">
          <div className="container mx-auto px-4">
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 mb-6">
              <div className="flex justify-between items-center mb-6">
                ...
                <div className="relative w-64">
                  ...
                  <Input type="text" placeholder="Search models..." className="pl-10" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
                </div>
              </div>
              ...
              {isLoading ? <div className="animate-pulse space-y-3">
                ...
                </div> : currentModels.length > 0 ? <div className="overflow-x-auto">
                  <Table>
                    <TableHeader>
                      <TableRow>
                    ...
                    <TableBody>
                      {currentModels.map(model => <TableRow key={model.id} className="hover:bg-orange-50 dark:hover:bg-orange-900/10">
